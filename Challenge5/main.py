...
def breakPass (motPass) :
    with open(r"mots.txt","r") as fp :    #On ouvre le document avec les possibles mdp
        encr = [hashlib.md5,hashlib.sha256,hashlib.sha512] #Liste avec les méthodes d'encryption qu'on utilisera
        ...
        return ""
###PROGRAME###
print("Veuillez écrire un mot de passe faible : ")  #Définition du mdp
motPass = input()

print("Selectionnez une méthode d'encryption : \n\t1) md5\n\t2) sha256\n\t3) sha512") #Définition de la méthode d'encryption
sel = int(input())
if sel == 3 :
